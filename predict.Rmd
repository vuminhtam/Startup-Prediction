...
#which results in 113k entries with total 15 features
df <- na.omit(df)
```
...
library('class')
knn_model <- knn(train = train_Set, test = test_Set, train_Set$encode_state, k = 11)
...
prediction <- predict(lin_model, type = "response", newdata = active_df)
pred_values <- ifelse(prediction > 0.5,1,0) 
...
percentage_succesful <- nrow(good_proj) / n
